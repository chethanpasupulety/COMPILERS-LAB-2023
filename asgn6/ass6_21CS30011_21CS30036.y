...
%union {
    ...
    dataTypes types;                 // For the type of an expression
    ...
    vector<param*> *prmList;        // For a list of params
}
...
// Non-terminals of type dataTypes (denoting types)
...
primary_expression: 
        ...
        {
            $$ = new expression();                  // Create new expression
            $$->loc = symTab->gentemp(INT);             // Generate a new temporary variable
            emit($$->loc, $1, ASSIGN);
            symbolValue* val = new symbolValue();
            val->setInitVal($1);                    // Set the initial value
            symTab->lookup($$->loc)->initVal = val;     // Store in symbol table
        }
        | FLOATING_CONSTANT
        {
            $$ = new expression();                  // Create new expression
            $$->loc = symTab->gentemp(FLOAT);           // Generate a new temporary variable
            emit($$->loc, $1, ASSIGN);
            symbolValue* val = new symbolValue();
            val->setInitVal($1);                    // Set the initial value
            symTab->lookup($$->loc)->initVal = val;     // Store in symbol table
        ...
        {
            $$ = new expression();                  // Create new expression
            $$->loc = symTab->gentemp(CHAR);            // Generate a new temporary variable
            emit($$->loc, $1, ASSIGN);
            symbolValue* val = new symbolValue();
            val->setInitVal($1);                    // Set the initial value
            symTab->lookup($$->loc)->initVal = val;     // Store in symbol table
...
postfix_expression: 
        ...
        {
            symbolType to = symTab->lookup($1->loc)->type;      // Get the type of the expression
            string f = "";
            if(!($1->fold)) {
        ...
        {   
            ...
            symbolTable* funcTable = symTabGlobal.lookup($1->loc)->nestedTable;
            emit($1->loc, "0", "", CALL);
        }
        ...
        {   
            ...
            symbolTable* funcTable = symTabGlobal.lookup($1->loc)->nestedTable;
            vector<param*> params = *($3);                          // Get the list of params
            ...
            if(retType == VOID)                                         // If the function returns void
                emit($1->loc, (int)params.size(), CALL);
            else {                                                      // If the function returns a value
                string retVal = symTab->gentemp(retType);
                emit($1->loc, to_string(params.size()), retVal, CALL);
                $$ = new expression();
                $$->loc = retVal;
            }
        }
        ...
        {   
            $$ = new expression();                                          // Create new expression
            symbolType t = symTab->lookup($1->loc)->type;                       // Get the type of the expression and generate a temporary variable
            if(t.type == ARRAY) {
                $$->loc = symTab->gentemp(symTab->lookup($1->loc)->type.nextType);
                emit($$->loc, $1->loc, *($1->folder), ARR_IND_BRAC);
                string temp = symTab->gentemp(t.nextType);
                emit(temp, $1->loc, *($1->folder), ARR_IND_BRAC);
                emit(temp, temp, "1", ADD);
                emit($1->loc, temp, *($1->folder), ARR_IND_ANS);
            }
            else {
                $$->loc = symTab->gentemp(symTab->lookup($1->loc)->type.type);
        ...
        {
            $$ = new expression();                                          // Create new expression
            $$->loc = symTab->gentemp(symTab->lookup($1->loc)->type.type);          // Generate a new temporary variable
            symbolType t = symTab->lookup($1->loc)->type;
            if(t.type == ARRAY) {
                $$->loc = symTab->gentemp(symTab->lookup($1->loc)->type.nextType);
                string temp = symTab->gentemp(t.nextType);
                emit(temp, $1->loc, *($1->folder), ARR_IND_BRAC);
...
unary_expression: 
        ...
        {
            $$ = new expression();
            symbolType type = symTab->lookup($2->loc)->type;
            if(type.type == ARRAY) {
                string t = symTab->gentemp(type.nextType);
                emit(t, $2->loc, *($2->folder), ARR_IND_BRAC);
                emit(t, t, "1", ADD);
                emit($2->loc, t, *($2->folder), ARR_IND_ANS);
                $$->loc = symTab->gentemp(symTab->lookup($2->loc)->type.nextType);
            }
            else {
                emit($2->loc, $2->loc, "1", ADD);                       // Increment the value
                $$->loc = symTab->gentemp(symTab->lookup($2->loc)->type.type);
            }
            $$->loc = symTab->gentemp(symTab->lookup($2->loc)->type.type);
        ...
        | unary_operator cast_expression
        {
            if($1 == '&'){
                $$ = new expression();
                $$->loc = symTab->gentemp(POINTER);                 // Generate temporary of the same base type
                emit($$->loc, $2->loc, "", REFERENCE);          // Emit the quad
            }
            else if($1 == '*'){
                $$ = new expression();
                $$->loc = symTab->gentemp(INT);                     // Generate temporary of the same base type
                $$->fold = 1;
                ...
                $$ = new expression();
                $$->loc = symTab->gentemp();                        // Generate temporary of the same base type
...
multiplicative_expression: 
        cast_expression
        {
            ...
            symbolType tp = symTab->lookup($1->loc)->type;
            if(tp.type == ARRAY) {                                  // If the type is an array
                string t = symTab->gentemp(tp.nextType);                // Generate a temporary
                if($1->folder != NULL) {
                    emit(t, $1->loc, *($1->folder), ARR_IND_BRAC);   // Emit the necessary quad
                    $1->loc = t;
                    $1->type = tp.nextType;
                    $$ = $1;
                }
        ...
        {   
            ...
            $$ = new expression();
            symbol* one = symTab->lookup($1->loc);                  // Get the first operand from the symbol table
            symbol* two = symTab->lookup($3->loc);                  // Get the second operand from the symbol table
            if(two->type.type == ARRAY) {                       // If the second operand is an array, perform necessary operations
                ...
                emit(t, $3->loc, *($3->folder), ARR_IND_BRAC);
                $3->loc = t;
            ...
            if(one->type.type == ARRAY) {                       // If the first operand is an array, perform necessary operations
                ...
                emit(t, $1->loc, *($1->folder), ARR_IND_BRAC);
                $1->loc = t;
            ...
            $$->loc = symTab->gentemp(final);                       // Store the final result in a temporary
...
relational_expression: 
        ...
        | relational_expression LESS_THAN shift_expression
        {
            ...
            emit("", $1->loc, $3->loc, GOTO_LT);                // Emit "if x < y goto XXXX"
            emit($$->loc, "0", "", ASSIGN);
        ...
        | relational_expression GREATER_THAN shift_expression
        {
            ...
            emit($$->loc, "1", "", ASSIGN);
            $$->truelist = makelist(nextInstr);                 // Set the truelist to the next instruction
            emit("", $1->loc, $3->loc, GOTO_GT);                // Emit "if x > y goto XXXX"
            emit($$->loc, "0", "", ASSIGN);
            $$->falselist = makelist(nextInstr);                // Set the falselist to the next instruction
            emit("", "", "", GOTO);                             // Emit "goto XXXX"
        }
        | relational_expression LESS_THAN_EQUAL shift_expression
        {
            ...
            emit($$->loc, "1", "", ASSIGN);
            $$->truelist = makelist(nextInstr);                 // Set the truelist to the next instruction
            emit("", $1->loc, $3->loc, GOTO_LTE);               // Emit "if x <= y goto XXXX"
            emit($$->loc, "0", "", ASSIGN);
            $$->falselist = makelist(nextInstr);                // Set the falselist to the next instruction
            emit("", "", "", GOTO);                             // Emit "goto XXXX"
        }
        | relational_expression GREATER_THAN_EQUAL shift_expression
        {
            ...
            emit($$->loc, "1", "", ASSIGN);
            $$->truelist = makelist(nextInstr);                 // Set the truelist to the next instruction
            emit("", $1->loc, $3->loc, GOTO_GTE);               // Emit "if x >= y goto XXXX"
            emit($$->loc, "0", "", ASSIGN);
            $$->falselist = makelist(nextInstr);                // Set the falselist to the next instruction
            emit("", "", "", GOTO);                             // Emit "goto XXXX"
        }
...
equality_expression: 
        ...
        | equality_expression EQUAL relational_expression
        {
            ...
            emit($$->loc, "1", "", ASSIGN);
            $$->truelist = makelist(nextInstr);                 // Set the truelist to the next instruction
            emit("", $1->loc, $3->loc, GOTO_EQ);                // Emit "if x == y goto XXXX"
            emit($$->loc, "0", "", ASSIGN);
            $$->falselist = makelist(nextInstr);                // Set the falselist to the next instruction
            emit("", "", "", GOTO);                             // Emit "goto XXXX"
        }
        | equality_expression NOT_EQUAL relational_expression
        {
            ...
            emit($$->loc, "1", "", ASSIGN);
            $$->truelist = makelist(nextInstr);                 // Set the truelist to the next instruction
            emit("", $1->loc, $3->loc, GOTO_NEQ);               // Emit "if x != y goto XXXX"
            emit($$->loc, "0", "", ASSIGN);
            $$->falselist = makelist(nextInstr);                // Set the falselist to the next instruction
            emit("", "", "", GOTO);                             // Emit "goto XXXX"
        }
...
conditional_expression: 
        ...
        {   
            ...
            symbol* one = symTab->lookup($5->loc);
            $$->loc = symTab->gentemp(one->type.type);      // Create a temporary for the expression
            $$->type = one->type.type;
...
assignment_expression: 
        ...
        {
            symbol* sym1 = symTab->lookup($1->loc);         // Get the first operand from the symbol table
            symbol* sym2 = symTab->lookup($3->loc);         // Get the second operand from the symbol table
            if($1->fold == 0) {
                if(sym1->type.type != ARRAY)
                    emit($1->loc, $3->loc, "", ASSIGN);
                else
                    emit($1->loc, $3->loc, *($1->folder), ARR_IND_ANS);
            }
            else
...
direct_declarator: 
        ...
        | direct_declarator SQUARE_BRACE_OPEN type_qualifier_list_opt assignment_expression SQUARE_BRACE_CLOSE
        {
            ...
            $1->nextType = INT;     // Array of ints
            $$ = $1;
            int index = symTab->lookup($4->loc)->initVal->i;
            $$->li.push_back(index);
        }
        ...
        | direct_declarator PARENTHESIS_OPEN parameter_type_list_opt PARENTHESIS_CLOSE
        {
            ...
            symbol* funcData = symTab->lookup($$->name, $$->type);
...
parameter_list: 
        ...
        {
            $$ = new vector<param*>;         // Create a new vector of params
...
jump_statement: 
        ...
        {
            if(symTab->lookup("RETVAL")->type.type == symTab->lookup($2->loc)->type.type) {
